...
def prepare_data(target_emotion='anger', other_emotions=None):
    """ Prepares Train, Validation, and Test set with vocabulary for target emotion """
    ...
    # .08 since the datasize is large enough.
    train_df, val_test_df = train_test_split(dataset, test_size=0.2, random_state=2018)
...
def load_embedding(word_index, embedding_file):
    """Create an embedding matrix which keeps only the embeddings for words in word_index """
...
def train_model(model):
    ...
    checkpoint = tf.keras.callbacks.ModelCheckpoint(
        filepath, monitor='val_loss', verbose=2, save_best_only=True, mode='min')
    reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(
        monitor='val_loss', factor=0.5, patience=1, min_lr=0.0001, verbose=2)
    earlystopping = tf.keras.callbacks.EarlyStopping(
        monitor='val_loss', min_delta=0.0001, patience=2, verbose=2, mode='auto')
